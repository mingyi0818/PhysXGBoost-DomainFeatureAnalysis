...
> A morphological feature engineering approach for classifying dry bean varieties. Domain features derived from bean shape, size, and structure measurements are compared against raw features across tree-based models for a multi-class classification task.

**Task**: Classification (Multi-class) | **Target**: class (7 bean varieties) | **Primary Metric**: AUC

## Dataset

| Item | Detail |
|------|--------|
| Name | Dry Bean (UCI ML Repository) |
| File | `data/drybean.csv` |
| Size | 0.29 MB |
| Source | UCI Machine Learning Repository - Dry Bean Dataset. 13,611 samples with 17 features (12 dimensions + 4 shape factors) for 7 bean varieties, with images processed using computer vision techniques. |
...
|--------|-----------|-------|
| AUC | XGBoost (Domain features) | AUC = 0.9839 |
