# Reproduction Guide: 64_FlightDelay
...
**Dataset**: Flight Delay (source TBD)
...
**Local path**: `data/ (directory missing - data file needs to be obtained)`

**Size**: N/A (data file not present)

CRITICAL: The data/ directory is missing. Results show AUC near 1.0, suggesting potential data leakage. Data must be obtained before reproduction.
...
Ensure the dataset file is present at `data/ (directory missing - data file needs to be obtained)`.
...
4. Records test-set AUC for every configuration.
...
1. Open `results/summary.json` and verify the AUC values for Raw and Domain feature sets.
...
|----------------|-------------|--------------|
| Abstract / Table | summary.json | Raw.*.AUC, Domain.*.AUC |
...
> CRITICAL: The data/ directory is missing. The results/summary.json shows AUC scores near 1.0 for all models, which is suspiciously high and likely indicates data leakage or an incorrect experiment setup. Data must be obtained and experiments re-run before any results can be trusted.
