...
> **CRITICAL**: No data file is present. The `data/` directory does not exist.

Please obtain the dataset from: Unknown - no data file is present in the data/ directory. The data/ directory is missing entirely.

Create the `data/` directory and place the dataset file there before running experiments.
